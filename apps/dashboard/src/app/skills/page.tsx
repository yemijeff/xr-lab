...
import { Award, Star, Plus, Minus } from 'lucide-react';
import { Skill } from '@xrlab/types';

export default function SkillsPage() {
  const [skills, setSkills] = useState<Skill[]>([]);
  const [loading, setLoading] = useState(true);
  // Track in-flight save requests per skill
  ...
  const handleUpdateLevel = (skillId: string, currentLevel: number, delta: number) => {
    const nextLevel = Math.max(1, Math.min(5, currentLevel + delta));

    // 1. OPTIMISTIC UPDATE — update UI instantly, no waiting
    setSkills((prev) =>
      prev.map((s) =>
        s.id === skillId ? { ...s, level: nextLevel, confidence: nextLevel } : s
      )
    );

    // 2. DEBOUNCE — cancel any pending save for this skill, then save after 400ms
    if (savingRef.current[skillId]) clearTimeout(savingRef.current[skillId]);

    savingRef.current[skillId] = setTimeout(async () => {
      try {
        ...
        if (data.success) {
          // Sync server truth (in case another device changed something)
          setSkills(data.data);
        }
      } catch (err) {
        ...
        setSkills((prev) =>
          prev.map((s) =>
            s.id === skillId ? { ...s, level: currentLevel, confidence: currentLevel } : s
          )
        );
      }
    }, 400);
  ...
  if (loading) {
    return <div className="p-12 text-center text-xs font-mono text-slate-500">Loading skills...</div>;
  }
